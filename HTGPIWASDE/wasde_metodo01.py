...
def upload_azure():
    ...
    day = '11'
    filemonth = 'wasde'+month+right(year,2)
    fname = year+month+day   
    url = 'https://www.usda.gov/oce/commodity/wasde/'+filemonth+'.xls'
    
    #Verificar si el archivo ya existe en WASDE
    if exist_fileurl(url)==False:
        return f'No existe informacion de WASDE del mes {fname}.'

    #se elimina la informacion del mes actual en caso ya se haya cargado una vez
    delete_wasde(fname)
    
    name = 'WASDE_'+fname+'.xls'
    ...
    file_download = extraer_datos(req_read,fname)
    block_blob_service.create_blob_from_text(container_name,name,file_download)
    return f'Extraccion de WASDE del mes {fname} fue exitosa.'

def delete_wasde(archivo):
    ...
    cursor = cnxn.cursor()
    sql = "delete from [ST_WASDE].[STOCKS_TO_USE] where archivo = '"+archivo+"' "
    cursor.execute(sql)
    cursor.commit()

def extraer_datos(req_read,fname):
